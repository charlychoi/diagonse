...
export function computeCoreReadiness(signals: ParsedSiteSignals): ScoreCard {
  const checks: AdaptiveCheck[] = [
    ...
    { id: "core-robots", title: "robots.txt 실측", status: signals.robotsTxt == null ? "not_observed" : signals.robotsTxt.disallowAll ? "fail" : signals.robotsTxt.found ? "pass" : "warn", detail: signals.robotsTxt == null ? "robots.txt를 확인하지 못했습니다." : signals.robotsTxt.disallowAll ? "⚠️ 전체 크롤링 차단(Disallow: /) — 검색 노출이 막혀 있습니다." : signals.robotsTxt.found ? "robots.txt 정상 응답" : "robots.txt가 없습니다.", action: signals.robotsTxt?.disallowAll ? "즉시 robots.txt의 전체 차단을 해제하세요 — 최우선 조치입니다." : "robots.txt에 사이트맵 위치를 선언하세요." },
    { id: "core-sitemap", title: "sitemap.xml 실측", status: signals.sitemapFound === undefined ? "not_observed" : signals.sitemapFound ? "pass" : "warn", detail: signals.sitemapFound ? "사이트맵 확인됨" : "sitemap.xml이 확인되지 않습니다.", action: "사이트맵을 생성하고 네이버 서치어드바이저·구글 서치콘솔에 제출하세요." },
