...
export async function callXaiApi(
...
): Promise<XaiApiResult> {
  const apiKey = options.apiKey || process.env.XAI_API_KEY;
  if (!apiKey) throw new Error("XAI_API_KEY가 설정되지 않았습니다. 프로젝트의 .env.local에 키를 설정해 주세요.");
  const model = options.model || process.env.XAI_MODEL || "grok-4.5";
  const response = await (options.fetchImpl || fetch)("https://api.x.ai/v1/responses", {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    ...
    throw new Error(typeof nested === "string" ? nested : `xAI API 오류 (${response.status})`);
  }
  const output = extractText(raw).trim();
  if (!output) throw new Error("xAI API 응답에 분석 텍스트가 없습니다.");
  return { provider: "xai", model, output, citations: collectCitations(raw) };
}
